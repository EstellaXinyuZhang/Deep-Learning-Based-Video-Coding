...
def optical_flow(im1_4, im2_4, batch, h, w):
    ...
    im1_3 = tf.layers.average_pooling2d(im1_4, pool_size=2, strides=2, padding='same') # (4, 128, 128, 3)
    im1_2 = tf.layers.average_pooling2d(im1_3, pool_size=2, strides=2, padding='same') # (4, 64, 64, 3)
    im1_1 = tf.layers.average_pooling2d(im1_2, pool_size=2, strides=2, padding='same') # (4, 32, 32, 3)
    im1_0 = tf.layers.average_pooling2d(im1_1, pool_size=2, strides=2, padding='same') # (4, 16, 16, 3)
    ...
    loss_0, flow_0 = loss(flow_zero, im1_0, im2_0, 0) # () (4, 16, 16, 2)
    loss_1, flow_1 = loss(flow_0, im1_1, im2_1, 1) # () (4, 32, 32, 2)
    loss_2, flow_2 = loss(flow_1, im1_2, im2_2, 2) # () (4, 64, 64, 2)
    loss_3, flow_3 = loss(flow_2, im1_3, im2_3, 3) # () (4, 128, 128, 2)
    loss_4, flow_4 = loss(flow_3, im1_4, im2_4, 4) # () (4, 256, 256, 2)
