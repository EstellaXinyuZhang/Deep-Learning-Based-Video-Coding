...
Y0_com = tf.placeholder(tf.float32, [batch_size, Height, Width, Channel]) # (4,256,256, 3)
Y1_raw = tf.placeholder(tf.float32, [batch_size, Height, Width, Channel])
learning_rate = tf.placeholder(tf.float32, [])

with tf.variable_scope("flow_motion"):

    flow_tensor, _, _, _, _, _ = motion.optical_flow(Y0_com, Y1_raw, batch_size, Height, Width) # (4, 256, 256, 2)
    # Y1_warp_0 = tf.contrib.image.dense_image_warp(Y0_com, flow_tensor)

# Encode flow
flow_latent = compressor.MV_analysis(flow_tensor, args.N, args.M) # (4, 16, 16, 128)
...
save_path = os.path.join(session_dir, './OpenDVC_PSNR_' + str(l))
