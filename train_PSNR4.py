...
flow_tensor, _ = motion.nn(img_pair)

# Encode flow
flow_latent = compressor.MV_analysis(flow_tensor, args.N, args.M)
...
flow_hat = compressor.MV_synthesis(flow_latent_hat, args.N)
...
res_latent = compressor.Res_analysis(Res, num_filters=args.N, M=args.M)
...
Res_hat = compressor.Res_synthesis(res_latent_hat, num_filters=args.N)

# Reconstructed frame
Y1_com = Res_hat + Y1_MC

# Refine flow
with tf.variable_scope("mv_refine"):
    flow_hat_re = refine_net.mv_refine_net(flow_hat)

# Reconstructed frame refine
with tf.variable_scope("rf_refine"):
    Y1_co_re = refine_net.rf_refine_net(Y1_com)
...
for tvar in tvars:
    if "mv_refine" not in tvar.name and "rf_refine" not in tvar.name:
        update_var_list.append(tvar)
print(update_var_list)
'''
train_ME = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(train_loss_ME, global_step=step)
...
tf.summary.scalar('train_loss_MV', train_loss_MV)
tf.summary.scalar('train_loss_MC', train_loss_MC)
tf.summary.scalar('train_loss_total', train_loss_total)
