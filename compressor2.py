  ...
  with tf.variable_scope("MV_analysis"):
    with tf.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
          2, (3, 3), corr=True, strides_down=2, padding="same_zeros",
          use_bias=True, activation=tfc.GDN())
      tensor = tf.layers.conv2d(inputs=tensor, filters=num_filters, kernel_size=[3, 3],
  ...
  with tf.variable_scope("analysis", reuse=reuse):
    with tf.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
          ...
          use_bias=True, activation=tfc.GDN())
      tensor = tf.layers.conv2d(inputs=tensor, filters=3, kernel_size=[3, 3],
                                padding="same", strides=1, activation=tf.nn.relu)
      tensor = layer(tensor)
